/**
 * In-memory token store for bulk (game-extras) ZIP downloads.
 *
 * Bulk ZIPs are ephemeral — no need to persist in the DB.
 * Tokens expire after 15 min (same as regular download tokens).
 *
 * NOTE: This feature is intentionally hidden from the UI.
 * The endpoint exists at GET /api/download/game-extras/[gameId]?type=dlc|update|mod&token=bulk_xxx
 * but no button exposes it to end users.
 */
